...
class LocalOpaPolicyTest {
    ...
    void allowsOwnerSelfAccessForNormalPrivacyScope() throws Exception {
        var response = evaluate("""
                {
                  "input": {
                    "actor": {"user_id": "user-1", "roles": ["user"], "scopes": []},
                    "action": "read",
                    "purpose": "self",
    ...
    @Test
    void allowsCrossUserReadForAuthorizedScope() throws Exception {
        var response = evaluate("""
                {
                  "input": {
                    "actor": {"user_id": "user-1", "roles": ["user"], "scopes": ["pkb:read:any"]},
                    "action": "read",
                    "purpose": "operations",
                    "resource": {"owner_user_id": "user-2", "privacy_scope": "restricted"}
                  }
                }
                """);
