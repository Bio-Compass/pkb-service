...
| LOCAL-012 | PKB item lookup                      | After seeding a `pkb_item`, export `TOKEN` to a BioCompass access token that is active at the configured auth introspection endpoint, then run `curl -fsS -H "Authorization: Bearer ${TOKEN}" 'http://localhost:8080/api/pkb/items/<item-id>?userId=<user-id>'`                                              | Response contains the requested `itemId`, `userId`, envelope fields, payload, scopes, and timestamps. |
| LOCAL-013 | PKB item search                      | After seeding `pkb_item` rows, export `TOKEN` to a BioCompass access token that is active at the configured auth introspection endpoint, then run `curl -fsS -H "Authorization: Bearer ${TOKEN}" 'http://localhost:8080/api/pkb/items?userId=<user-id>&entityType=observation&status=active&text=hydration'` | Response contains only matching items for the authenticated user scope.                               |
...
| LOCAL-015 | PKB cross-user query denial          | Export `TOKEN` to a non-staff BioCompass access token for one user, then run `curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer ${TOKEN}" 'http://localhost:8080/api/pkb/items?userId=<different-user-id>'`                                                                                   | Response code is `403`.                                                                               |
