...
| AUTO-004 | CI deploy change classifier   | `./gradlew testCiScripts --no-daemon`                                                                                                         | Classifier allows image-only Helm diffs without approval regardless of changed source files, requires approval for non-image Helm and suppressed Secret changes, and writes a manual approval review with the full Helm diff. |
